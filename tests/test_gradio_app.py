...
def test_chatbot_uses_messages_type():
    """Verify that the demo contains a Chatbot with a placeholder configured.

    Note: Gradio 5+ made the messages format the only format (no legacy tuples),
    so there is no longer a ``type`` parameter to check. We verify the Chatbot
    is present and configured with a placeholder instead.
    """
    demo = create_chat_interface()
    ...
    assert chatbot.placeholder, \
        "Chatbot should have a placeholder configured"


def test_chatbot_has_copy_button():
    """Verify that the Chatbot component is present in the demo.

    Note: Gradio 5+ removed the ``show_copy_button`` parameter — copy
    behaviour is now controlled at the app level. We verify the Chatbot
    component itself is present and correctly configured with a height.
    """
    demo = create_chat_interface()
    ...
    assert chatbot is not None, "Chatbot component not found"
    assert chatbot.height, \
        "Chatbot should have a height configured"
